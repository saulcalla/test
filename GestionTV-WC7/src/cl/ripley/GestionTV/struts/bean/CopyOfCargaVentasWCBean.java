 ...
 public class CopyOfCargaVentasWCBean extends ConectaBD
 {
   ...
   public void setSucursalVenta(DataSource dataSource, String orderId)
   {
     ...
     try
     {
       ...
       if (!existe)
         setMens_error("ERROR: Al obtener sucursal de venta");
     }
     catch (Exception e)
     {
     ...
     if (conecta(dataSource)) {
       try {
         ...
         if ((vRes != null) && (vRes.size() > 0))
         {
           ...
           vInstruccion = (String)Util.getMap().get("gestion_70.inst02");
           ...
           String dv_cli = this.rutStr.substring(id + 1, this.rutStr.length());
           if (dv_cli.toUpperCase().equals("K"))
             vInstruccion = vInstruccion + "'" + dv_cli + "', ";
           else {
             vInstruccion = vInstruccion + "'" + dv_cli.substring(0, 1) + "', ";
           }
           ...
           if (id > 0)
           {
             setCod_regalo(str_cd_regalo.substring(id + 1, str_cd_regalo.length()));
     ...
     if (conecta(dataSource)) {
       try
       {
         ...
         if ((vRes != null) && (vRes.size() > 0))
         {
           ...
           vWhere.add("Int," + idDirDespacho);
           consultaRst(dataSource, vInstruccion, vWhere);
           dirDespacho = filtra(getRst(), 12);
 
           vInstruccion = (String)Util.getMap().get("gestion_71.inst02");
 
           vInstruccion = vInstruccion + orderId + ", ";
 
           vInstruccion = vInstruccion + "0, ";
 
           String nombre_desp = dirDespacho.get(1).toString() + " " + dirDespacho.get(2).toString() + " " + dirDespacho.get(3).toString();
 
           if (nombre_desp.length() >= 50)
             vInstruccion = vInstruccion + "'" + nombre_desp.substring(0, 50) + "', ";
           else {
           ...
           String direccion_desp = dirDespacho.get(6).toString() + " " + dirDespacho.get(7).toString() + " " + dirDespacho.get(8).toString();
 
           if (direccion_desp.length() >= 50) {
             vInstruccion = vInstruccion + "'" + direccion_desp.substring(0, 50) + "', ";
           }
           else {
             vInstruccion = vInstruccion + "'" + direccion_desp + "', ";
           }
 
           vInstruccion = vInstruccion + "'" + dirDespacho.get(10).toString() + "', ";
 
           vInstruccion = vInstruccion + "'S', ";
 
           if (comentario.size() > 0)
             vInstruccion = vInstruccion + "'" + comentario.get(0).toString() + "', ";
           else {
           ...
           if (vRes.size() > 5) {
             direccion_cli = vRes.get(6).toString() + " " + vRes.get(7).toString() + " " + vRes.get(8).toString() + " " + vRes.get(9).toString();
           }
 
           if (direccion_cli.length() >= 50)
             vInstruccion = vInstruccion + "'" + direccion_cli.substring(0, 50) + "', ";
           else {
             vInstruccion = vInstruccion + "'" + direccion_cli + "', ";
           }
 
           if (vRes.size() > 11)
             vInstruccion = vInstruccion + "'" + vRes.get(10).toString() + "', ";
           else {
             vInstruccion = vInstruccion + "'', ";
           }
           vInstruccion = vInstruccion + "0, ";
           ...
           if (vRes.size() > 11)
             vInstruccion = vInstruccion + "'" + vRes.get(2).toString() + "', ";
           else {
             vInstruccion = vInstruccion + "'', ";
           }
 
           if (vRes.size() > 11)
             vInstruccion = vInstruccion + "'" + vRes.get(3).toString() + "', ";
           else {
             vInstruccion = vInstruccion + "'', ";
           }
           ...
           if (vRes.size() > 11)
             vInstruccion = vInstruccion + "'" + vRes.get(11).toString() + "', ";
           else {
             vInstruccion = vInstruccion + "'', ";
           }
     ...
     if (conecta(dataSource)) {
       try
       {
         ...
         while (rstProductos.next()) {
           vInstruccion = (String)Util.getMap().get("gestion_72.inst01");
           for (int i = 1; i < 15; i++)
           {
             String value = "";
             ...
             if (i == 3)
             {
               ...
               {
                 StringTokenizer tokens = new StringTokenizer(partnum, "|");
             ...
             if (i == 4) {
               ...
               vWhere.add("Int," + rstProductos.getString(14).toString());
               consultaRst(dataSource, sql, vWhere);
 
               v_desc = filtra(getRst(), 1);
               ...
               if (v_desc.size() == 0)
               {
                 ...
                 if (v_desc.size() == 0) {
                   ...
                   vInstruccion = vInstruccion + "'" + v_desc.get(0).toString() + "',";
                 }
               }
               else
               {
             ...
             if (i == 5) {
               vInstruccion = vInstruccion + rstProductos.getString(i).toString() + ",";
             }
         ...
         if (correlativo > 1)
         {
           ...
           while (rstProductos.next())
           {
             ...
             desc_precio = rstProductos.getString("desc_costo_envio").trim();
             ...
             sql = (String)Util.getMap().get("gestion_73.inst02");
             vWhere.add("String," + reg_com);
 
             vRes = consulta(dataSource, sql, vWhere, 4);
             for (int i = 0; i < vRes.size(); i++)
             {
               String[] s_envio = vRes.get(i).toString().split(",");
               if (tipo_desp.equals("SML")) {
                 String val = s_envio[0];
                 val = val.substring(1);
         ...
         log.error("::Exception Al genera SQL INSERT ARTICULO_VENTA " + e.getMessage());
         return null;
       } finally {
         cierraRst();
       }
     ...
     if (conecta(dataSource)) {
       try {
         ...
         if ((vRes != null) && (vRes.size() > 0))
         {
           ...
           vInstruccion = (String)Util.getMap().get("gestion_73.inst03");
       ...
       {
         log.error("::Exception Al genera SQL INSERT Tarjeta Bancaria" + e);
         return null;
       } finally {
         cierraRst();
       }
   ...
   public String getSQLTarjetaRipley(DataSource dataSource, String orderId)
   {
     ...
     SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd 00:00:00.000000");
     ...
     if (conecta(dataSource)) {
       try {
         ...
         while (getRst().next())
         {
           if (getRst().getString("name").trim().equals("CAR_EMBOZADO")) {
             car_embozado = getRst().getString("value");
           }
           if (getRst().getString("name").trim().equals("CAR_RUT")) {
             car_rut = getRst().getString("value");
             car_embozado = car_embozado + getRst().getString("value");
           }
           if (getRst().getString("name").trim().equals("CAR_DIFERIDO")) {
             car_rut = getRst().getString("value");
             car_embozado = car_embozado + getRst().getString("value");
           }
           ...
           if (getRst().getString("name").trim().equals("dv")) {
             dv_rut = getRst().getString("value");
           }
           if (getRst().getString("name").trim().equals("CAR_VALOR_CUOTA")) {
             car_valor_cuota = getRst().getString("value");
           }
           if (getRst().getString("name").trim().equals("CAR_FECHA_VENC")) {
   ...
   public int cargaVentas(DataSource dataSource)
   {
     ...
     if (conecta(dataSource)) {
       try
       {
         ...
         setMens_error("Error al cargar tablas." + e.getLocalizedMessage());
         intRes = -1;
       }
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.bean.CopyOfCargaVentasWCBean
