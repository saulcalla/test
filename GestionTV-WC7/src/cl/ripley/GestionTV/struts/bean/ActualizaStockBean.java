 ...
 public class ActualizaStockBean extends ConectaBD
 {
   ...
   public int SqlBorradoDePromociones(DataSource dataSource, Vector vectorSql)
   {
     int intRes = 0;
     String statement = "";
     ...
     try
     {
       ...
       {
         statement = (String)Util.getMap().get("gestion_65.inst03");
         String cadena = null;
         ...
         for (int i = 0; i < vectorSql.size(); i++)
         {
           ...
           if (cadena.equals("PRODUCTO|STOCK"))
           {
             cadena = "";
           }
           else
           {
             ...
             if (ind > 0) {
               ...
               sql_delete = statement.replaceAll("PRODUCTO", padre);
 
               sql_delete = sql_delete.replaceAll("STOCK", hijo);
   ...
   public Vector lecturaExcel(FormFile myFile)
   {
     ...
     try {
         ...
         cells.hasNext())
       {
         ...
         case 0:
           setMens_error("ERROR: Valor[" + cell.getNumericCellValue() + "] debe ser tipo de dato 'TEXTO'");
           return null;
         case 1:
           if (concatenar) {
     ...
     catch (Exception e)
     {
       ...
       setMens_error("ERROR: al leer excel");
       return null;
     }
   ...
   public int actualizarStock(DataSource dataSource)
   {
     int intRes = 0;
     if (conecta(dataSource)) {
       try {
         if (getVectDelete().size() > 0)
           ...
           setMens_error("Problemas al actualizar stock.\nCargue nuevamente el archivo.");
       }
       catch (Exception e) {
         log.error("::Exception, EN actualizar Stock " + e.toString());
         setMens_error("Error al actualizar Stock.");
         intRes = -1;
       }
       finally {
         cierraRst();
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.bean.ActualizaStockBean
