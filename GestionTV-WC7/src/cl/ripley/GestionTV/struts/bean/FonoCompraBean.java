 ...
 public class FonoCompraBean extends ConectaBD
 {
   ...
   public Vector getPaginacion(DataSource dataSource, String fecini, String fecfin) {
     Vector vRes = new Vector();
     int maxIndice = getIndice() + getRegistrosPorBloque() + 1;
     String statement = (String)Util.getMap().get("gestion_03.inst03");
     if (conecta(dataSource)) {
       ...
       vRes = consulta(dataSource, statement, vWhere, 8);
       cierraRst();
     }
   ...
   public int getTotalRegistros(DataSource dataSource, String fecini, String fecfin) {
     Vector vRes = new Vector();
     int intRes = 0;
     String statement = (String)Util.getMap().get("gestion_08.inst01");
   ...
   public Vector getListaPagina(DataSource dataSource, String fecini, String fecfin) {
     Vector v = new Vector();
 
     int p = getTotalPaginas(dataSource, fecini, fecfin);
   ...
   public int getTotalPaginas(DataSource dataSource, String fecini, String fecfin) {
     int p = 0;
 
     int r = getRegistrosPorBloque();
 
     if (r != 0) {
       p = getTotalRegistros(dataSource, fecini, fecfin) / r;
       int b = p * r;
 
       if (b != getTotalRegistros(dataSource, fecini, fecfin))
       {
         p++;
       }
     }
   ...
   public Vector getVectorExportar(DataSource dataSource)
   {
     Vector vRes = new Vector();
     String statement = 
       (String)Util.getMap().get("gestion_10.inst01");
     if (conecta(dataSource)) {
       Vector vWhere = new Vector();
       vWhere.add("Date," + getFec_inicio().substring(6, 10) + "-" + getFec_inicio().substring(3, 5) + "-" + getFec_inicio().substring(0, 2));
       vWhere.add("Date," + getFec_fin().substring(6, 10) + "-" + getFec_fin().substring(3, 5) + "-" + getFec_fin().substring(0, 2));
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.bean.FonoCompraBean
