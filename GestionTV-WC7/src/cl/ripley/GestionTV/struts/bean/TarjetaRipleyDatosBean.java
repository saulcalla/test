 ...
 public class TarjetaRipleyDatosBean extends ConectaBD
 {
   ...
   public Vector getPaginacion(DataSource dataSource) {
     ...
     String statement = (String)Util.getMap().get("gestion_24.inst04");
   ...
   public int getTotalRegistros(DataSource dataSource) {
     Vector vRes = new Vector();
     int intRes = 0;
     String statement = (String)Util.getMap().get("gestion_25.inst01");
   ...
   public Vector getVectorExportar(DataSource dataSource)
   {
     Vector vRes = new Vector();
     String statement = 
       (String)Util.getMap().get("gestion_25.inst02");
     if (conecta(dataSource)) {
       Vector vWhere = new Vector();
       vWhere.add("Date," + getFec_inicio().substring(6, 10) + "-" + getFec_inicio().substring(3, 5) + "-" + getFec_inicio().substring(0, 2));
       vRes = consulta(dataSource, statement, vWhere, 7);
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.bean.TarjetaRipleyDatosBean
