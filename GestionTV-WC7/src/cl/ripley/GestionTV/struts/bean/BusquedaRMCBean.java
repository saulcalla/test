 ...
 public class BusquedaRMCBean extends ConectaBD
 {
   ...
   public Vector getPaginacion(DataSource dataSource, String nro_rmc)
   {
     ...
     if (conecta(dataSource))
     {
       try
       {
         vInstruccion = (String)Util.getMap().get("gestion_52.inst01");
         vWhere.add("String," + nro_rmc);
         vWhere.add("Int," + Integer.toString(getIndice()));
         vWhere.add("Int," + Integer.toString(maxIndice));
 
         vRes = consulta(dataSource, vInstruccion, vWhere, 13);
       }
       catch (Exception e) {
   ...
   public int getTotalRegistros(DataSource dataSource, String nro_rmc)
   {
     Vector vRes = new Vector();
     int intRes = 0;
     String statement = "";
     ...
     try {
       if (conecta(dataSource))
       {
         statement = (String)Util.getMap().get("gestion_52.inst02");
         vWhere.add("String," + nro_rmc);
   ...
   public Vector getVectorExportar(DataSource dataSource, String nro_rmc)
   {
     Vector vRes = new Vector();
     String statement = "";
     Vector vWhere = new Vector();
     try
     {
       if (conecta(dataSource))
       {
         statement = (String)Util.getMap().get("gestion_52.inst03");
         vWhere.add("String," + nro_rmc);
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.bean.BusquedaRMCBean
