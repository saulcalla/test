 ...
 import cl.ripley.GestionTV.struts.bean.VentasXusuarioBean;
 import cl.ripley.GestionTV.struts.form.VentasXusuarioForm;
 ...
 public class VentasXusuarioAction extends Action
 {
     ...
     throws Exception
   {
     ...
     VentasXusuarioForm conForm = (VentasXusuarioForm)form;
     VentasXusuarioBean conBean = new VentasXusuarioBean();
 
     String idUsuario = (String)request.getSession().getAttribute("strRutUsr");
 
     if (conForm.getAccion().equals("InicioVentasXusuario")) {
       conForm.setVctDetalles(null);
       conForm.setHayDatos("no");
       conForm.setRespuesta("");
       conForm.setHayDatosExportar("no");
       conForm.setFec_inicio("");
       conForm.setFlagDatosBD("si");
 
       conForm.setCollEjecutivoFiltro(conBean.getCollEjecutivos(getDataSource(request, "Db2")));
       conForm.setSelectEjecutivoFiltro("");
     ...
     if (conForm.getAccion().equals("buscar"))
     {
       ...
       conBean.setSelecEjecutivo(conForm.getSelectEjecutivoFiltro());
     ...
     if (conForm.getAccion().equals("actualizar"))
     {
       ...
       conBean.setRegistrosPorBloque(conForm.getRegistrosPorBloque());
       conBean.setIndice(conForm.getIndice());
       conBean.setVentas(conForm.getVentasTC());
       int resp = conBean.actualizaRegistros(getDataSource(request, "Db2"));
 
       if (resp >= 0) {
         conForm.setRespuesta("Registros Actualizados.");
       }
       else {
         conForm.setRespuesta("Error al actualizar registros.");
       }
 ...
 * Qualified Name:     cl.ripley.GestionTV.struts.action.VentasXusuarioAction
