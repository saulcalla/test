...
document.write('<link rel="stylesheet" type="text/css" href="pages/styles/estylemenu.css">');
document.write('<link rel="stylesheet" type="text/css" href="styles/estylemenu.css">');
...
* Fecha: 13-10-2003 11:07
* Objetivo: Permite quitar espacios a la izquierda y derecha de la cadena
*/
...
function crea_menu(cadena)
{
	/**
	* Autor: Patricio González
	* Fecha: 10-10-2003
	* Objetivo: Permite transformar el tipo de nivel de el enviado
	* por el servlet y el que maneja el menu.
	*/
	...
	while (posicion!=-1)
	{
		cadena = cadena.substring(posicion+2);
		poscoma = cadena.indexOf(',');
		alert(poscoma);
		vNivel= trim(cadena.substring(0,poscoma));
		//vNivel= trim(cadena.substring(0,5));
		vDescripcion= trim(cadena.substring(9,39));
		vPagina= trim(cadena.substring(42,102));
		...
		if (vNivelMenu!="")
		{
			//slideMenu.makeMenu(vNivelMenu,vDescripcion,vPaginaMenu);
			slideMenu.makeMenu(vNivelMenu,vDescripcion,vPaginaMenu,'derecho');
		}
		posicion = cadena.indexOf('[');			
	}
	slideMenu.init();		
